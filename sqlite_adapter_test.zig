...
test "in-memory adapter loads canonical snapshots and compare-and-sets state" {
    var adapter = try sqlite.Adapter.open(":memory:", .{});
    defer adapter.close();
...
test "file adapter persists state across connections" {
    ...
    {
        var first = try sqlite.Adapter.open(database_path, .{});
        defer first.close();
        var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
        defer arena.deinit();
    ...
    {
        var second = try sqlite.Adapter.open(database_path, .{});
        defer second.close();
        var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
        defer arena.deinit();
...
test "busy timeout maps lock contention to unavailable" {
    ...
    defer std.testing.allocator.free(database_path);
    var adapter = try sqlite.Adapter.open(database_path, .{
        .busy_timeout_ms = 1,
    });
    defer adapter.close();
