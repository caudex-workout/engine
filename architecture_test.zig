...
test "public root exports are intentional" {
    ...
    try std.testing.expectEqual(@as(usize, 5), declarations.len);
    ...
    try std.testing.expectEqualStrings("methodology", declarations[2].name);
    try std.testing.expectEqualStrings("primitives", declarations[3].name);
    try std.testing.expectEqualStrings("training", declarations[4].name);
}
