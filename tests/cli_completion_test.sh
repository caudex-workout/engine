...
test_diagnostics_init "CLI completion"
...
printf '%s\n' \
  ...
  '_caudex() {' \
  '    local commands="database doctor workout set exercise history program config batch completion command-reference"' \
  '    COMPREPLY=( $(compgen -W "$commands" -- "${COMP_WORDS[COMP_CWORD]}") )' \
  '}' \
  'complete -F _caudex caudex' >"$temporary_directory/expected-bash"
printf '%s\n' \
  '#compdef caudex' \
  "_arguments '1:command:(database doctor workout set exercise history program config batch completion command-reference)'" >"$temporary_directory/expected-zsh"
printf '%s\n' \
